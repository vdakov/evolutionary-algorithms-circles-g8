...
if __name__ == "__main__":
    ...
    result_directory = os.path.join(
        "outputs",
        f'{datetime.now().strftime("%Y%m%d_%H%M%S")}_hyperparameter_optimiser',
    )
    ...
    while indexes is not None:
        ...
        def do_instance(instance_seed, current_parameters, results_manager, max_evals):
            ...
            runner = CirclesInASquare(
                ...
                init_strategy=InitializationStrategy.from_string(
                    current_parameters["initialization_strategy"]
                ),
            ...
            best_solution = runner.run_evolution_strategies(
                ...
                constraint_handling=ConstraintHandling.from_string(
                    current_parameters["constraint_handling"]
                ),
                max_evaluations=max_evals,
                recombination_strategy=RecombinationStrategy.from_string(
                    current_parameters["recombination_strategy"]
                ),
        ...
        if parallelize:
            with multiprocess.Pool(len(seeds)) as pool:
                results = pool.starmap(
                    do_instance,
                    [
                        (s, current_parameters, results_manager, max_evals)
                        for s in seeds
                    ],
                )
        else:
            for seed in seeds:
                results.append(
                    do_instance(seed, current_parameters, results_manager, max_evals)
                )

        end_time = time.time()
        print(
            f"Finished experiment {current_experiment}/{total_experiments} in {end_time - start_time} seconds"
        )
        ...
        with open(
            os.path.join(
                result_directory,
                f"results_{str(current_experiment).zfill(len(str(total_experiments)))}.json",
            ),
            "w",
        ) as f:
