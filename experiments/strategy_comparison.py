...
def run_strategy_comparison(
    ...
    # Setup
    strategies = [Strategy.SINGLE, Strategy.MULTIPLE, Strategy.FULL_VARIANCE]
    if random_seeds is None:
        random_seeds = np.random.randint(0, 1000000, size=n_runs)
    # Create results manager for the experiment
    results_manager = ResultsManager("strategy_comparison", save_files=False)
    # Results storage
    ...
    for strategy in strategies:
        strategy_results = results[str(strategy)]
        fitnesses = [r["best_fitness"] for r in strategy_results]
        target = strategy_results[0]["target_value"]  # Same for all runs
        analysis[str(strategy)] = {
    ...
    data = [
        [r["best_fitness"] for r in results[str(strategy)]] for strategy in strategies
    ]
    plt.boxplot(data, tick_labels=[s.value for s in strategies])
    plt.axhline(y=target, color="r", linestyle="--", label="Target Value")
    plt.title("Strategy Comparison")
    plt.ylabel("Best Fitness Achieved")
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(results_manager.run_dir, "strategy_comparison.png"))
    plt.close()
    # Enhanced elbow plot
    plot_combined_elbow(
        results,
        strategies,
        os.path.join(results_manager.run_dir, "strategy_progression.png"),
    )
...
if __name__ == "__main__":
    ...
    results, analysis = run_strategy_comparison(
        n_circles=10, n_runs=5, population_size=30, num_children=1, generations=50
    )
    # Print summary
