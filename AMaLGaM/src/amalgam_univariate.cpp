 ...
 * with a univariately factorized covariance matrix
 *
...
void estimateVarianceVectorML( int population_index );
...
** variance_vectors,                 /* The variance vectors to be used for sampling, one for each population. */
...
void initializeMemory( void )
{
  ...
  variance_vectors             = (double **) Malloc( number_of_populations*sizeof( double * ) );
  ...
  for( i = 0; i < number_of_populations; i++ )
  {
    ...
    variance_vectors[i] = (double *) Malloc( number_of_parameters*sizeof( double ) );
  }
...
void initializePopulationsAndFitnessValues( void )
{
  ...
  // @todo change initializtion to put points more evenly spread in the box
  for( i = 0; i < number_of_populations; i++ )
  {
    for( j = 0; j < population_size; j++ )
    {
...
/**
 * Estimates the parameters of the multivariate
 * normal distribution for each population separately.
 */
void estimateParametersAllPopulations( void )
{
  int i;
 ...
 * for a specificied population.
 */
void estimateParametersML( int population_index )
{
  estimateMeanVectorML( population_index );
  
  estimateVarianceVectorML( population_index );
}

/**
 * Computes the sample mean for a specificied population.
 */
void estimateMeanVectorML( int population_index )
{
  int i, j;
...
/**
 * Computes the vector of sample variances for
 * a specificied population.
 *
...
void estimateVarianceVectorML( int population_index )
{
  ...
  for( i = 0; i < number_of_parameters; i++ )
  {
    variance_vectors[population_index][i] = 0.0;
    
    for( j = 0; j < selection_size; j++ )
      variance_vectors[population_index][i] += (selections[population_index][j][i]-mean_vectors[population_index][i])*(selections[population_index][j][i]-mean_vectors[population_index][i]);
    
    variance_vectors[population_index][i] /= (double) selection_size;
  }
}

/**
 * Copies the selected solutions to their respective populations.
 */
void copyBestSolutionsToPopulations( void )
{
  int i, j, k;
...
void applyDistributionMultipliers( void )
{
  ...
  for( i = 0; i < number_of_populations; i++ )
  {
    ...
    {
      for( j = 0; j < number_of_parameters; j++ )
        variance_vectors[i][j] *= distribution_multipliers[i];
    }
  }
}
...
double *generateNewSolution( int population_index )
{
  ...
  for( i = 0; i < number_of_parameters; i++ )
  {
    ...
    do
    {
      ...
      out_of_bounds_draws[population_index]++;
      result[i] = random1DNormalParameterized( mean_vectors[population_index][i], variance_vectors[population_index][i] );
      
      if( times_not_in_bounds >= 100 )
        result[i] = lower_init_ranges[i] + (upper_init_ranges[i] - lower_init_ranges[i])*randomRealUniform01();
    }
    while( !isParameterInRangeBounds( problem_index, result[i] ) );
  }
 ...
 * of a given point for a given distribution.
 */
double getStDevRatio( int population_index, double *parameters )
{
  int    i;
  double ratio, difference, result;
  
  result = 0.0;
  for( i = 0; i < number_of_parameters; i++ )
  {
    if( variance_vectors[population_index][i] > 0 )
    {
      difference = parameters[i] - mean_vectors[population_index][i];
      ratio      = fabs( difference ) / sqrt( variance_vectors[population_index][i] );
...
/*-=-=-=-=-=-=-=-=-=-=-=-=- Section Ezilaitini -=-=-=-=-=-=-=-=-=-=-=-=-=-=-*/
/**
 * Frees memory that was allocated upon initialization.
 */
void ezilaitini( void )
{
  ezilaitiniMemory();
...
void ezilaitiniMemory( void )
{
  ...
  for( i = 0; i < number_of_populations; i++ )
  {
    ...
    free( variance_vectors[i] );
  }
  
  free( variance_vectors );
...
int main( int argc, char **argv )
{
  ...
  interpretCommandLine( argc, argv, false, parameters );
